...
const Servers: React.FC = () => {
  ...
  const fetchServerData = async (server: ServerInfo): Promise<Partial<ServerInfo>> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        // Simular datos básicos sin información de mapa
        ...
        const ping = Math.floor(Math.random() * 30) + 15; // Ping realista para Chile
  ...
  const getServerStatusColor = (server: ServerInfo) => {
    if (!server.isOnline) return 'text-red-400';
    return 'text-green-400';
  };

  const getServerStatusText = (server: ServerInfo) => {
    if (!server.isOnline) return 'Desconectado';
    return 'En línea';
  };

  const getPingColor = (ping: number) => {
    if (ping >= 999) return 'text-red-400';
    if (ping >= 100) return 'text-yellow-400';
    if (ping >= 50) return 'text-blue-400';
    return 'text-green-400';
  };
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-right">
                  <div className="flex items-center space-x-1 mb-1">
                    <TrendingUp className={`w-4 h-4 ${getPingColor(server.ping)}`} />
                    <span className={`text-lg font-bold ${getPingColor(server.ping)}`}>
                      {server.ping >= 999 ? 'N/A' : `${server.ping}ms`}
                    </span>
                  </div>
